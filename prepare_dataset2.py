	...
	for root, dirs, files in os.walk(root_directory):
	    for filename in files:
	        if filename.endswith(('.wav', '.au')):
	            ...
	            if dir_label in label_list:
	            	file_dict[dir_label].append(os.path.relpath(os.path.join(root, filename), '.'))
	            	labelled = True
	            else:
	            	for label in label_list:
	            		...
	            		if match:
	            			...
	            			labelled = True
	            			break
	            if labelled is not True:
	            	warnings.warn('Found audio file %s, but could not determine its label' % filename)
	os.chdir(pwd)
